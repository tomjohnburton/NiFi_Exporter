...
def metrics():
    ...
    for item in cluster:
        ...
        activeThreadCount = Gauge('nifi_node_activeThreadCount', 'Total number of activeThreadCount per node',
                                  NodeName.keys(), registry=registry)
        queuedItems = Gauge('nifi_node_queuedItems', 'Number of queud items per node', NodeName.keys(),
                            registry=registry)
    ...
    #### ====  info about failed flowfiles ==== ####
    url = BASE_URL + "/nifi-api/connections/" + CONNECTION_UUID
    nifi_retry_flowfile_count = getRetryCount(url)
    nifiRetryCount = Gauge('nifi_retry_flowfile_count', 'Total number of flowfiles in retry queue', registry=registry)
    nifiRetryCount.set(nifi_retry_flowfile_count)
    lst.append(prometheus_client.generate_latest(nifiRetryCount))
    ...
    for PG in nifi_group:
        ...
        processorQueue = Gauge('nifi_amount_items_queued', 'Total number of items queued by the processor',
                               processorName.keys(), registry=registry)
